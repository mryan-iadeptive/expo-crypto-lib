/**
 * BIP39-like mnemonic generation and seed derivation.
 * Uses node-forge for hashing; optional IRandomValues for entropy.
 */
...
const wordlist = [
  ...
  'nod', 'noise', 'nominee', 'noodle', 'normal', 'north', 'nose', 'notable', 'note', 'nothing',
  ...
  'sense', 'sentence', 'series', 'service', 'seven', 'shadow', 'shaft', 'shallow', 'share', 'shed',
  ...
  'switch', 'sword', 'symbol', 'symptom', 'syrup', 'system', 'table', 'tail', 'talent', 'talk',
  ...
  'upright', 'uprising', 'uproar', 'upset', 'upstairs', 'upward', 'urban', 'urge', 'usage', 'use',
  ...
  'vow', 'voyal', 'vulnerable', 'wade', 'wagon', 'wait', 'walk', 'wall', 'walnut', 'want',
  ...
  'wet', 'whale', 'what', 'whatever', 'wheat', 'wheel', 'when', 'whenever', 'where', 'whereas', 'wherever',
...
function defaultRandomValues(): IRandomValues {
  const c = typeof globalThis !== 'undefined' && (globalThis as any).crypto;
  if (c && typeof c.getRandomValues === 'function') {
    return {
      getRandomValues<T extends ArrayBufferView | null>(array: T): T {
        return c.getRandomValues(array) as T;
      },
    };
  }
  throw new Error('MnemonicManager.generateMnemonic: provide randomAdapter or ensure crypto.getRandomValues is available (e.g. globalThis.crypto)');
}

export class MnemonicManager {
  /** Generate a 24-word mnemonic (256 bits of entropy). Pass randomAdapter in Expo/RN or when crypto is not global. */
  static async generateMnemonic(randomAdapter?: IRandomValues): Promise<string> {
    const rand = randomAdapter ?? defaultRandomValues();
    const entropy = new Uint8Array(32);
    rand.getRandomValues(entropy);
    return this.entropyToMnemonic(entropy);
  }

  private static entropyToMnemonic(entropy: Uint8Array): string {
    const totalBits = entropy.length * 8 + entropy.length * 8 / 32;
    const words: string[] = [];
    ...
    for (let i = 0; i < wordCount; i++) {
      ...
      while (bitsRead < 11) {
        const byteIndex = startByte + Math.floor((startBitInByte + bitsRead) / 8);
        const bitIndex = (startBitInByte + bitsRead) % 8;
  ...
  /** Derive seed from mnemonic (SHA512 of mnemonic + salt). */
  static async mnemonicToSeed(mnemonic: string, passphrase: string = ''): Promise<Uint8Array> {
    const combinedData = mnemonic + 'mnemonic' + passphrase;
    const md = forge.md.sha512.create();
    md.update(combinedData, 'utf8');
    const digest = md.digest();
    const hex = digest.toHex();
    const seedArray = new Uint8Array(hex.length);
    for (let i = 0; i < hex.length; i++) {
      seedArray[i] = hex.charCodeAt(i);
    }
    return seedArray;
  }
  ...
  static validateMnemonic(mnemonic: string): boolean {
    ...
    return words.length === 24 && words.every((word) => wordlist.includes(word));
  }
}
